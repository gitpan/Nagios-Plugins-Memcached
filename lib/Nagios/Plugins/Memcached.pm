...
sub new {
    ...
    my %args = (
        shortname => 'MEMCACHED',
        usage => 'Usage: %s [-H host] [-w warnings] [-c critical] [--size-warnng size-warnng] [--size-critical size-critical] [--hit-warning hit-warning] [--hit-critical hit-critical] [-t timeout] [-v] [-h] [-?] [-V] [--extra-opts section@config_file]',
        version => $VERSION,
        url => 'http://search.cpan.org/dist/Nagios-Plugins-Memcached/bin/check_memcache',
        license => qq|This library is free software, you can redistribute it and/or modify\nit under the same terms as Perl itself.|,
    );
...
sub check_size {
	...
	my $use_size = $host_stats->{bytes} * 100 / $host_stats->{limit_maxbytes};
	...
	my $code = $self->check_threshold(
        ...
        $self->add_message($code, "Size checked: NG - at $host") if ($code > OK);
...
sub check_hit {
    ...
    for my $host (keys %{$stats->{hosts}}) {
        ...
        if ($host_stats->{cmd_get} == 0) {
            $self->add_message(OK, "Hit checked: OK - stats cmd_get is zero at $host");
            next;
        }
        ...
        $self->add_message($code, "Hit checked: NG - at $host") if ($code > OK);
...
sub detect_runmodes {
    ...
    if ($opts->get("warning") && $opts->get("critical")) {
        $self->nagios_exit(UNKNOWN, "Invalid arguments - warning > critical") if ($opts->get("warning") > $opts->get("critical"));
    ...
    if ($opts->get("hosts") && @{$opts->get("hosts")} == 1 && $opts->get("size-warning") && $opts->get("size-critical")) {
        $self->nagios_exit(UNKNOWN, "Invalid arguments - size-warning > size-critical") if ($opts->get("size-warning") > $opts->get("size-critical"));
    ...
    if ($opts->get("hit-warning") && $opts->get("hit-critical")) {
        $self->nagios_exit(UNKNOWN, "Invalid arguments - hit-warning < hit-critical") if ($opts->get("hit-warning") < $opts->get("hit-critical"));
